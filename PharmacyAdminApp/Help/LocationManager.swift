...
final class LocationManager: NSObject {
    ...
    typealias ReverseGeoLocationAddressClosure = ((_ address: String) -> ())
    private var locationAddresssCompletionHandler: ReverseGeoLocationAddressClosure?
    ...
    func getReverseGeoCodedAddressLocation(location:CLLocationCoordinate2D, completionHandler:@escaping ReverseGeoLocationAddressClosure) {
    ...
    func getReverseGeoCodedAddressLocation(location: CLLocationCoordinate2D, completionHandler: @escaping (_ address: String,_ country: String) -> ()) {
        let geocoder = GMSGeocoder()
        geocoder.accessibilityLanguage = "en-US"
        geocoder.reverseGeocodeCoordinate(location) { response , error in
            if let address = response?.firstResult() {
                let lines = address.lines! as [String]
                completionHandler("\(lines.joined(separator: ", "))", address.country ?? "")
            }
        }
    ...
    func calculateDistanceBetween(startLocation:CLLocation,endLocation:CLLocation)  ->Double {
